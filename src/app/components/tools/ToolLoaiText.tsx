...
export default function ToolLoaiText() {
    ...
    const handleTrigger = () => {
        ...
        if (cutLength > 0) {
            lines = lines.filter(l => l.length !== cutLength);
        }
        if (removeContains) {
            lines = lines.map(l => l.replaceAll(removeContains, ''));
        }
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <div className="flex items-center">
                <label className="text-gray-700 font-medium w-48 text-right me-3">
                    Cắt nội dung có length
                </label>
                <input type="number" className="border rounded-md px-3 py-1 w-32" value={cutLength} onChange={(e) => setCutLength(Number(e.target.value))} />
            </div>
            <div className="flex flex-row items-center">
                <label className="text-gray-700 font-medium w-48 text-right me-3">Xoá nội dung chứa</label>
                <input type="text" placeholder="Để trống nếu không muốn xoá" className="border rounded p-2 flex-1" value={removeContains} onChange={(e) => setRemoveContains(e.target.value)} />
            </div>
            <div>
