    ...
    const handleTrigger = () => {
        ...
        for (let i = 0; i < lines.length; i++) {
            if (lines[i].startsWith("email=")) {
                const email = lines[i].replace("email=", "").trim();
                const passLine = lines[i + 1] || "";
                const pass = passLine.replace("pass=", "").trim();
                if (email && pass) {
                    resultArr.push(`${email}${separator}${pass}`);
                }
            }
        }
        let items = resultArr;
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <div className="flex flex-row items-center">
                <label className="text-gray-700 font-medium w-48 text-right me-3">Ngăn cách bởi ký tự</label>
