...
export default function ToolJoinLines() {
    const [removeDuplicates, setRemoveDuplicates] = useState(true);
    const [mode, setMode] = useState<JoinMode>('all');
    ...
    const handleTrigger = () => {
        ...
        if (mode === 'all') {
            ...
            output = processedLines.join(delimiter);
        } else if (mode === 'consecutive') {
            // Ghép các dòng liên tiếp theo cặp
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-2">
                ...
                <select
                    ...
                    className="w-full p-3 border border-[var(--border)] rounded-lg bg-[var(--background)] text-[var(--foreground)] text-sm focus:outline-none focus:ring-2 focus:ring-[var(--primary)] focus:border-transparent"
                >
                    <option value="all">Tất các dòng</option>
                    <option value="consecutive">Liên tiếp</option>
                </select>
            </div>
